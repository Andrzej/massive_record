  ...
  describe "with an open connection" do
    ...
    describe "and a new initialized table with column families" do 
      ...
      it "should be able to do atomic increment call on values" do
        row = @table.first
        row.atomic_increment("misc:value_to_increment").should == 1
        row.atomic_increment("misc:value_to_increment",0).should == 1

        result = row.atomic_increment("misc:value_to_increment")
        result.should == 2
      end

      it "should be able to pass inn what to increment by" do
        row = @table.first
        row.atomic_increment("misc:value_to_increment",0).should == 2
        row.atomic_increment("misc:value_to_increment", 2).should == 4

        row = @table.first
        row.atomic_increment("misc:value_to_increment",0).should == 4
      end
